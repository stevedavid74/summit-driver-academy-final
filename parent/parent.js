(() => {
  ...
  async function loadPortal(user){
    ...
    if(ids.length){
      const [{data:studentData,error:studentError},{data:reportData,error:reportError}]=await Promise.all([
        client.from('students').select('id,full_name,preferred_language,status,online_hours,classroom_hours,flexible_hours,in_car_hours,summit_score,road_test_readiness,current_focus,next_lesson_at,training_week_start').in('id',ids).order('full_name'),
  ...
  function render(){
    ...
    $('#classroomProgress').value=Number(student.classroom_hours||0);$('#classroomValue').textContent=`${Number(student.classroom_hours||0)}/20 ${copy[lang].hours}`;
    $('#flexibleProgress').value=Number(student.flexible_hours||0);$('#flexibleValue').textContent=`${Number(student.flexible_hours||0)}/10 ${copy[lang].hours}`;
