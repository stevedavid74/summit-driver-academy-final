(() => {
  ...
  function render(){
    ...
    $('#reportList').innerHTML=visible.map(report=>`<article class="report"><header><h3>${copy[lang].lesson} · ${dateOnly(report.lesson_date)}</h3><small>${Number(report.duration_minutes)} ${copy[lang].minutes}</small></header>${report.parent_summary?`<p>${escapeHtml(report.parent_summary)}</p>`:''}<div class="scores">${scorePill(copy[lang].overall,report.overall_score)}${scorePill(copy[lang].observation,report.observation_score)}${scorePill(copy[lang].intersections,report.intersections_score)}${scorePill(copy[lang].lane,report.lane_control_score)}${scorePill(copy[lang].parking,report.parking_score)}${scorePill(copy[lang].defensive,report.defensive_driving_score)}</div></article>`).join('');
  }
